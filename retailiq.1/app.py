...
with tab2:
    ...
    st.caption(
        "First checks a library of 60 hand-written, verified SQL reports for a "
        "close match. If nothing fits well, you can ask an AI model to write a "
        "new query on the spot — useful for questions the library doesn't cover."
    )
    ...
    if user_q:
        ...
        if best_score >= CONFIDENT_THRESHOLD:
            ...
            st.divider()
            ask_ai_anyway = st.toggle("Not what you meant? Try an AI-generated query instead")
        else:
            st.info(
                "Nothing in the 60-query library matches this closely enough to trust "
                "automatically. You can still try an AI-generated query below."
            )
            ask_ai_anyway = True
        ...
        if ask_ai_anyway:
            ...
            if not api_key:
                st.warning(
                    "AI fallback isn't configured. Add a free `GROQ_API_KEY` "
                    "(get one at https://console.groq.com/keys) to your Streamlit "
                    "secrets to enable open-ended questions beyond the 60 built-ins."
                )
            else:
                with st.spinner("Asking the model to write a query..."):
                    try:
                ...
                if generated_sql and not safe:
                    ...
                    st.caption("🤖 AI-generated — double-check before relying on it.")
                    st.code(generated_sql, language="sql")
                    try:
